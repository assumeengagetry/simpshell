...
// helper
static Job *parse_job_arg(char **argv, const char *builtin_name,
                          int *job_id_out);

BuiltinCommand builtins[] = {{"cd", builtin_cd},     {"exit", builtin_exit},
                             {"pwd", builtin_pwd},   {"clear", builtin_clear},
                             {"jobs", builtin_jobs}, {"bg", builtin_bg},
                             {"fg", builtin_fg},     {NULL, NULL}};
...
int builtin_jobs(char **argv) {
  ...
  for (int i = 0; i < MAX_JOBS; i++) {
    ...
    // 模仿真实 Shell 的输出格式
    printf("[%d] %d %s\t\t%s\n", job->job_id, job->pgid,
           job->status == RUNNING ? "Running" : "Stopped", job->cmd);
  }
  return 0;
}
